...
class DataLoader():
    ...
    def __init__(self, f_prefix, batch_size=5, seq_length=20, num_of_validation = 0, forcePreProcess=False, infer=False, generate = False):
        ...
        # The valid fraction is set when we need to use part of a dataset as training data, part as valid data.
        self.val_fraction = 0
    ...
    def time_preprocess(self, data_dirs, data_file, validation_set = False):
        ...
        '''
        # The beginning-with-valid list is for valid-fraction mode,
        # In this mode, for a training dataset, part of it is for training and part of it is for valiation.
        # In validation/test mode and if we do not use valid-fraction mode, these data is empty.

        # all_timeStamp_data would be a list of list of numpy arrays corresponding to each dataset
        # Each numpy array will correspond to a timeStamp and would be of size (numShips, 3) each row
        # containing shipMmsi, lng, lat
        all_timeStamp_data = []
        # Validation frame data
        valid_timeStamp_data = []

        # timeStampList_data would be a list of lists corresponding to each dataset
        # Each list would contain the frameIds of all the timeStamps in the dataset
        timeStampList_data = []
        
        # numShips_data would be a list of lists corresponding to each dataset
        # Each list would contain the number of ships in each timeStamp in the dataset
        numShips_data = []
        valid_numShips_data = []

        # each list includes ship mmsi of this timestamp
        shipsList_data = []
        ...
        for directory in data_dirs:
            ...
            for ind, timeStamp in enumerate(timeStampList):
                ...
                if (ind >= numTimeStamp * self.val_fraction) or (self.infer) or (validation_set):
                    # Add the details of all the ships in the current frame to all_timeStamp_data
                    ...
                    # TODO There seems no change to add to valid data.
                    # The valid fraction is set when we need to use part of a dataset as training data, part as valid data.
    ...
    def load_preprocessed(self, data_file, validation_set = False):
        ...
        # counter for training sequence and validation sequence.
        counter = 0
        valid_counter = 0
    ...
    def next_valid_batch(self):
        ...
        This function is for valid-fraction mode, if we do not use it, ignore the function/
        '''
        # Source data
        x_batch = []
        # Target data
    ...
    def tick_batch_pointer(self, valid=False):
        ...
        '''
        if not valid:
            # Go to the next dataset for next_batch function.
            ...
            # Go to the next dataset for next_valid_batch function.
    ...
    def write_dataset(self, dataset_seq, file_name, path):
        ...
        np.savetxt(os.path.join(path, file_name), out, fmt = "%s,%s,%s,%s", newline='\n')
