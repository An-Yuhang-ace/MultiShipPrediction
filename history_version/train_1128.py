    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # TODO change the grid part 
                #dataset_data = dataloader.get_dataset_dimension(folder_name)

                #dense vector creation
                x_seq, lookup_seq = dataloader.convert_proper_array(x_seq, numShipsList_seq, shipsList_seq)
                #target_mmsi_values = x_seq[0][lookup_seq[target_mmsi], 0:2]
                ...
                #         grids[dataloader.dataset_pointer].append(grid_seq)
                #     else:
                #         #grid_seq1 = getSequenceGridMask(x_seq, dataset_data, shipsList_seq,args.neighborhood_size, args.grid_size, args.use_cuda)
                #         grid_seq = grids[dataloader.dataset_pointer][(num_batch*dataloader.batch_size)+sequence]
                ...
                #print(grid_seq)
                  ...
                  # Construct variables
                #print("target id : ", target_mmsi)
                #print("look up : ", lookup_seq)
                #print("pedlist_seq: ", shipsList_seq)
                #print("before_xseq: ", x_seq)
                #x_seq, target_mmsi_values, first_values_dict = vectorize_seq_with_ped(x_seq, shipsList_seq, lookup_seq ,target_mmsi)
                #print("after_vectorize_seq: ", x_seq)
                #print("angle: ", np.rad2deg(angle))
                #print("after_xseq: ", x_seq)
                #x_seq = rotate_traj_with_target_ped(x_seq, -angle, shipsList_seq, lookup_seq)
                #x_seq = revert_seq(x_seq, shipsList_seq, lookup_seq, first_values_dict)
                ...
                # Forward prop
                outputs, _, _ = net(x_seq, grid_seq, hidden_states, cell_states, shipsList_seq, numShipsList_seq, dataloader, lookup_seq)
        ...
        if dataloader.valid_num_batches > 0:
            ...
            for batch in range(dataloader.valid_num_batches):
                ...
                for sequence in range(dataloader.batch_size):
                    ...
                    # Forward prop
                    outputs, _, _ = net(x_seq[:-1], grid_seq[:-1], hidden_states, cell_states, shipsList_seq[:-1], numShipsList_seq , dataloader, lookup_seq)
