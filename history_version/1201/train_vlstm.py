...
def train(args):
    ...
    # Tensorboard
    writer1 = SummaryWriter('log/vlstm')
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            # Get batch data
            # x:[batch_size, seq_length, num_ships, 3], x:[batch_size, seq_length, num_ships, 3]
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # TODO dense vector creation
                # x_seq:[seq_length, num_ships, 3]
                ...
                # Forward prop
                outputs, _, _ = net(x_seq, hidden_states, cell_states, shipsList_seq, numShipsList_seq, dataloader, lookup_seq)

                # Compute loss
                loss = Gaussian2DLikelihood(outputs, x_seq, shipsList_seq, lookup_seq)
                if args.use_cuda:
                    loss = loss.cuda()
        ...
        writer1.add_scalar('train_loss', loss_epoch, global_step=epoch*num_batch+batch)


        # This part of code is for valid-fraction mode.
        # In this mode, for a training dataset, part of it is for training and part of it is for valiation.
        # If we do not use valid-fraction mode, these code can be ignored.
        if dataloader.valid_num_batches > 0:
            ...
            for batch in range(dataloader.valid_num_batches):
                # Get batch data
                x, y, d , numShipsList, shipsList ,target_mmsis= dataloader.next_valid_batch()
                ...
                for sequence in range(dataloader.batch_size):
                    ...
                    # Forward prop
                    outputs, _, _ = net(x_seq[:-1], hidden_states, cell_states, shipsList_seq[:-1], numShipsList_seq , dataloader, lookup_seq)
                    ...
                    next_x, next_y = sample_gaussian_2d(mux.data, muy.data, sx.data, sy.data, corr.data, shipsList_seq[-1], lookup_seq)
                    next_vals = torch.FloatTensor(1,numNodes,2)
                    next_vals[:,:,0] = next_x
                    next_vals[:,:,1] = next_y
                    err = get_mean_error(next_vals, x_seq[-1].data[None, : ,:], [shipsList_seq[-1]], [shipsList_seq[-1]], args.use_cuda, lookup_seq)
        ...
        if dataloader.additional_validation and (epoch) in validation_epoch_list:
            ...
            for batch in range(dataloader.num_batches):
                ...
                for sequence in range(dataloader.batch_size):
                    ...
                    ret_x_seq, loss = sample_validation_data_vanilla(x_seq, shipsList_seq, args, net, lookup_seq, numShipsList_seq, dataloader)
